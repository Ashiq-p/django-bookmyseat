...
from movies.views import personalized_recommendations


def home(request):
    movies = personalized_recommendations(request.user, limit=4)
...
def profile(request):
    ...
    payments = Payment.objects.filter(user=request.user).select_related('theater', 'theater__movie').order_by('-created_at')
    if request.method == 'POST':
